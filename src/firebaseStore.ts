...
export const DataStore = {
  ...
  getOrders: async (restaurantId?: string): Promise<Order[]> => {
    ...
    const q = query(
      collection(db, 'restaurants', id, 'orders'),
      orderBy('time', 'desc'),
      limit(100)
    );
  ...
  updateOrderStatus: async (id: string, status: Order['status'], restaurantId?: string) => {
    const resId = restaurantId || getRestaurantId();
    const docRef = doc(db, 'restaurants', resId, 'orders', id);
  ...
  getAvailableDeliveryOrders: async (restaurantId?: string): Promise<Order[]> => {
    const resId = restaurantId || getRestaurantId();
    const q = query(
      collection(db, 'restaurants', resId, 'orders'),
      orderBy('time', 'desc')
    );
    const snap = await getDocs(q);
    return snap.docs
  ...
  assignOrderToDriver: async (orderId: string, driverId: string, restaurantId?: string) => {
    const resId = restaurantId || getRestaurantId();
    const orderRef = doc(db, 'restaurants', resId, 'orders', orderId);
    await updateDoc(orderRef, { driverId });
    
    // Increment driver active orders
    const driverRef = doc(db, 'restaurants', resId, 'drivers', driverId);
  ...
  getProfile: async (restaurantId?: string): Promise<RestaurantProfile> => {
    ...
    if (snap.exists()) {
      return { id, ...snap.data() } as RestaurantProfile;
    }
    return { id, name: 'Mon Restaurant', description: '', coverImage: '', logo: '' };
  },
  ...
  subscribeToProfile: (callback: (profile: RestaurantProfile) => void, restaurantId?: string) => {
    ...
    return onSnapshot(docRef, (snap) => {
      if (snap.exists()) {
        callback({ id, ...snap.data() } as RestaurantProfile);
      } else {
        callback({ id, name: 'Mon Restaurant', description: '', coverImage: '', logo: '' });
      }
    });
  },
  ...
  getStaff: async (restaurantId?: string): Promise<StaffMember[]> => {
    const id = restaurantId || getRestaurantId();
    const q = collection(db, 'restaurants', id, 'staff');
    const snap = await getDocs(q);
  ...
  getDrivers: async (restaurantId?: string): Promise<Driver[]> => {
    const id = restaurantId || getRestaurantId();
    const q = collection(db, 'restaurants', id, 'drivers');
    const snap = await getDocs(q);
  ...
  updateDriverStatus: async (id: string, status: Driver['status'], restaurantId?: string) => {
    const resId = restaurantId || getRestaurantId();
    const docRef = doc(db, 'restaurants', resId, 'drivers', id);
    await updateDoc(docRef, { status });
  },

  updateDriverOrders: async (id: string, count: number, restaurantId?: string) => {
    const resId = restaurantId || getRestaurantId();
    const docRef = doc(db, 'restaurants', resId, 'drivers', id);
